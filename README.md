...
Surge:
https://raw.githubusercontent.com/demo1874/wloc/refs/heads/main/modules/wloc.sgmodule

Quantumult X:
https://raw.githubusercontent.com/demo1874/wloc/refs/heads/main/modules/wloc.conf

Loon:
https://raw.githubusercontent.com/demo1874/wloc/refs/heads/main/modules/wloc.lpx

Stash:
https://raw.githubusercontent.com/demo1874/wloc/refs/heads/main/modules/wloc.stoverride

Shadowrocket(小火箭):
https://raw.githubusercontent.com/demo1874/wloc/refs/heads/main/modules/wloc.module
...
- **wloc 设置地理位置**：https://www.icloud.com/shortcuts/2a811c3bac0e4694b1875ae45b26ab0d
- **wloc 清理恢复位置**：https://www.icloud.com/shortcuts/9092b1a576cf4b7c8bc0dd931ba512a6
...
为了让苹果地图和高德走同一条流程，链接统一发给 `wloc-spoofer.kinshanusai.workers.dev/api/parse` 解析：
...
- 部署后把快捷指令里的 `wloc-spoofer.kinshanusai.workers.dev` 换成你自己的 worker 域名即可。
...
**透传模式触发条件：** 持久化数据为空（null）且模块参数为默认值（-0.127800, 51.507900）时，脚本判定用户未自定义坐标，自动跳过修改。模块默认参数无需更改，仅清除持久化数据即可触发透传。
...
> **注意：** 如果用户在模块参数中手动修改了经纬度（非默认 51.5012,0.1915），即使清除持久化数据，脚本仍会使用模块参数中的坐标进行修改。只有保持默认参数不变时，清除持久化数据才会进入透传模式。
...
- **Workers**: `https://wloc-spoofer.kinshanusai.workers.dev/`

**一键部署（Workers）：**

[![Deploy to Cloudflare Workers](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/hankinsus/wloc/tree/main/worker)
...
```bash
# 1. 克隆仓库
git clone https://github.com/hankinsus/wloc.git
cd wloc/worker
...
- 小火箭定位配置生效（无需全局），可以在default.conf，在规则里面添加 `gs-loc.apple.com` 和 `gs-loc-cn.apple.com`
</details>
